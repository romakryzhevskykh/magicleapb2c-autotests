...
#    Then Order Successful pop-up appears at the OE 3 page.
  ...
  Scenario Outline: Check that Total Net Price value is correctly after changing status boxes
    ...
    And User clicks on Shipped status box.
    Then Is Shipped status box price equals to the total net price.
...
#    When User clicks on time status drop down field on PO Details page.
#    Then Is Opened PO Details block table contains correct header <tableTitles> titles.
