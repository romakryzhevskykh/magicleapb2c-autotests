...
#      | New! Download your open orders report -> | Without Pricing | rmautotest@gmail.com |
  ...
  Scenario Outline: Check that user can open Detail Orders page and verify main elements
    ...
    Then Is Ordered On label contains date.
    Then Is Order Value label contains USD value.
    Then Is Ship To label contains address.
    Then Is Created by label contains @ email character.
    Then Is Ship Method label contains text.
    Then Is Tax Total label contains USD value.
    Then Is User No. and Quote No. labels displayed.
    ...
    Then Is Correct header titles displayed in the table header.
    Then Sum of Ext.Price
