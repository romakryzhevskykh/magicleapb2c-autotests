...
public class NotificationCenterPage extends StorefrontBasePage {
    ...
    @Step("Get Promotional content Toggle Position.")
    public String getPromotionalContentTogglePosition() {
        String togglePosition;
        if ($(MARKETING_PROMOTIONAL_CONTENT_TOGGLE_XPATH).getCssValue("background-color").equals("rgba(59, 115, 185, 1)")) {
            togglePosition = "ON";
        } else {
            togglePosition = "OFF";
        }
    ...
    @Step("Switch The Promo Content Toggle.")
    public void switchThePromoContentToggle() {
        waitUntilPageIsFullyLoaded();
        click(MARKETING_PROMOTIONAL_CONTENT_TOGGLE_XPATH);
    }
}